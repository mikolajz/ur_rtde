...
use ur_rtde::raw_rtde::{RTDEData, GenericRawRTDE, RawRTDETrait};
...
fn test_raw_rtde() {
    ...
    let inputs_setup = rtde.configure_inputs(&["standard_digital_output_mask", "standard_digital_output"]).unwrap();
    println!("{:?}", inputs_setup);
    rtde.send_robot_input(&inputs_setup, &vec![
        RTDEData::Uint8(0x7),
        RTDEData::Uint8(0x3),
    ]).unwrap();

    let outputs_setup = rtde.configure_outputs(125f64, &["actual_TCP_speed", "actual_TCP_pose"]).unwrap();
