...
impl MuxWriteBuffer {
    pub(crate) fn new(init_values: &BTreeMap<String, RTDEData>) -> MuxWriteBuffer {
        MuxWriteBuffer {
    ...
    pub(crate) fn update(&mut self, names: &[String], values: &[RTDEData]) -> Result<(), String> {
        for (name, value) in names.iter().zip(values) {
            ...
            let previous_value = self
                ...
                .ok_or_else(|| format!("Unsupported name {name}"))?;
            if discriminant(&previous_value) != discriminant(value) {
                return Err(format!(
                    "Value {name} changed type from {previous_value:?} to {value:?}"
                ));
            }
        }
        Ok(())
...
mod tests {
    ...
    fn merges_values_correctly() {
        ...
        assert_eq!(
            buffer.values(),
            vec![RTDEData::Int32(1), RTDEData::Int32(2), RTDEData::Uint64(3)]
        );

        buffer
            .update(
                &[String::from("c"), String::from("a")],
                &[RTDEData::Uint64(4), RTDEData::Int32(5)],
            )
            .expect("update failed");
        assert_eq!(
            buffer.values(),
            vec![RTDEData::Int32(5), RTDEData::Int32(2), RTDEData::Uint64(4)]
        );

        buffer
            .update(
                &[String::from("a"), String::from("b")],
                &[RTDEData::Int32(6), RTDEData::Int32(7)],
            )
            .expect("update failed");
        assert_eq!(
            buffer.values(),
            vec![RTDEData::Int32(6), RTDEData::Int32(7), RTDEData::Uint64(4)]
        );
    }
    ...
    fn fails_on_type_change() {
        ...
        buffer
            .update(&[String::from("a")], &[RTDEData::Uint64(4)])
            .expect_err("unexpectedly succeeded to change the type");
    }
    ...
    fn fails_on_invalid_name() {
        ...
        buffer
            .update(&[String::from("x")], &[RTDEData::Uint64(4)])
            .expect_err("unexpectedly succeeded on wrong name");
    }
}
