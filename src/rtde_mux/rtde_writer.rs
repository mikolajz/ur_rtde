...
use crate::rtde_mux::{RTDEWriteVariableSet, MuxWriteBuffer};
    ...
    where U: RTDEWriteVariableSet
{
    ...
    pub fn send(&self, data: &U) -> Result<(), String> {
        ...
        shared_data.write_buffer.update(&self.variables, &data.convert_into_rtde_data())?;
        shared_data.rtde_thread.send(&shared_data.write_buffer.values())?;
        Ok(())
    }
}
...
mod tests {
    ...
    use crate::rtde_mux::test_variable_sets::tests::{WriteVariableSetCB, WriteVariableSetAB};
    ...
    fn test_writes_from_two_writers() {
        ...
        initial_values.insert(String::from("standard_analog_output_0"), RTDEData::Double(7.25));
        initial_values.insert(String::from("standard_digital_output"), RTDEData::Uint8(0));
        initial_values.insert(String::from("standard_digital_output_mask"), RTDEData::Uint8(0));

        let (fake_thread, fake_thread_control) = FakeRTDEThread::new();
        let fake_thread_ptr: Arc<dyn RTDEThreadTrait> = Arc::new(fake_thread);

        let shared_data = RTDEWriterSharedData::new(
            &fake_thread_ptr,
            &initial_values,
        );
        ...
        writer1.send(&WriteVariableSetCB{ standard_digital_output: 0x5, standard_digital_output_mask: 0xf }).unwrap();

        assert_eq!(fake_thread_control.borrow().sent_packets.len(), 1);
        assert_eq!(fake_thread_control.borrow().sent_packets[0].payload, vec![
            RTDEData::Double(7.25),
            RTDEData::Uint8(0x5),
            RTDEData::Uint8(0xf),
        ]);

        writer2.send(&WriteVariableSetAB{ standard_analog_output_0: 1.75, standard_digital_output_mask: 0x3f }).unwrap();

        assert_eq!(fake_thread_control.borrow().sent_packets.len(), 2);
        assert_eq!(fake_thread_control.borrow().sent_packets[1].payload, vec![
