...
use crate::rtde_mux::{
    RTDEReadVariableSet, RTDEReader, RTDEWriteVariableSet, RTDEWriter, RTDEWriterSharedData,
};
    ...
    T: 'static + RawRTDETrait + Send,
{
    ...
    pub fn new(raw_rtde: T, read_frequency: f64) -> RTDEMultiplexedConnectionBuilder<T> {
    ...
    pub fn add_read_variable_set<U: RTDEReadVariableSet>(
        &mut self,
    ) -> Result<RTDEReaderPromise<U>, String> {
    ...
    pub fn add_write_variable_set<U: RTDEWriteVariableSet>(
        &mut self,
        initial_values: &U,
    ) -> Result<RTDEWriterPromise<U>, String> {
    ...
    pub fn connect(self) -> Result<RTDEMultiplexedConnection, String> {
        let thread: Arc<dyn RTDEThreadTrait> = Arc::new(RTDEThread::start(
            self.raw_rtde,
            &self
                .write_variables
                .keys()
        ...
        let writer_shared_data = RTDEWriterSharedData::new(&thread, &self.write_variables);

        // BTreeSet doesn't seem to provide an easy way to get the index of an element, so let's build an index.
        let var_names_to_index = self
            .read_variables
            .iter()
            .enumerate()
            .map(|(i, var_name)| (var_name.clone(), i))
            .collect();
...
impl RTDEMultiplexedConnection {
    ...
    pub fn get_reader<U: RTDEReadVariableSet>(
        &self,
        promise: RTDEReaderPromise<U>,
    ) -> Result<RTDEReader<U>, String> {
        if promise.cookie != self.cookie {
            return Err(
                "The promise object is from a different RTDEMultiplexedConnection".to_string(),
            );
        }

        RTDEReader::new(&self.rtde_thread, &self.var_names_to_index)
    }
    ...
    pub fn get_writer<U: RTDEWriteVariableSet>(
        &self,
        promise: RTDEWriterPromise<U>,
    ) -> Result<RTDEWriter<U>, String> {
        if promise.cookie != self.cookie {
            return Err(
                "The promise object is from a different RTDEMultiplexedConnection".to_string(),
            );
    ...
    pub fn disconnect(self) -> Result<(), String> {
        ...
        if writers_left > 0 {
            return Err(format!(
                "Some writers are still active (count {writers_left}"
            ));
        }
        ...
        if readers_left > 0 {
            return Err(format!(
                "Some readers are still active (count {readers_left}"
            ));
        }
...
mod tests {
    ...
    use crate::rtde_mux::test_variable_sets::tests::{
        ReadVariableSetXY, ReadVariableSetYZ, WriteVariableSetAB, WriteVariableSetCB,
    };
    ...
    fn sets_inputs_and_outputs() {
        ...
        mux_builder
            .add_read_variable_set::<ReadVariableSetXY>()
            .unwrap();
        mux_builder
            .add_read_variable_set::<ReadVariableSetYZ>()
            .unwrap();
        ...
        assert_eq!(
            control.lock().unwrap().output_io_setups[0].vars,
            vec!["payload", "robot_mode", "timestamp"]
        );
    }
    ...
    fn can_read_packets() {
        ...
        let reader_xy_promise = mux_builder
            .add_read_variable_set::<ReadVariableSetXY>()
            .unwrap();
        let reader_yz_promise = mux_builder
            .add_read_variable_set::<ReadVariableSetYZ>()
            .unwrap();
        ...
        control.lock().unwrap().queue_repeating_output(
            0,
            &[
    ...
    fn can_write_packets() {
...

        {
            ...
            assert_eq!(
                control_data.input_io_setup.as_ref().unwrap().vars,
                vec![
                    "standard_analog_output_0",
                    "standard_digital_output",
                    "standard_digital_output_mask"
            ...
            assert_eq!(
                control_data.sent_input_packets,
                vec![
                    vec![
                        RTDEData::Double(1.0),
                        RTDEData::Uint8(1),
                        RTDEData::Uint8(1)
                    ], // Initial value sent without any call to send().
                    vec![
                        RTDEData::Double(7.0),
                        RTDEData::Uint8(1),
                        RTDEData::Uint8(7)
