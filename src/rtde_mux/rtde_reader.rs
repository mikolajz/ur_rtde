...
use crate::rtde_thread::{RTDEThreadTrait, PacketId};
...
impl<U: RTDEReadVariableSet> RTDEReader<U>
{
    ...
    pub fn get_latest_packet(&self, newer_than: &Option<PacketId>) -> Result<(PacketId, U), String> {
        let packet = self.rtde_thread.get_latest_packet(newer_than)?;
        let variable_set_data: Vec<RTDEData> = self.var_indices.iter().map(|i| packet.payload[*i].clone()).collect();
...
mod test {
    ...
    fn test_read_from_two_readers() {
        ...
        let reader1 = RTDEReader::<ReadVariableSetXY>::new(&fake_thread_ptr, &thread_variables_to_index_map).unwrap();
        let reader2 = RTDEReader::<ReadVariableSetYZ>::new(&fake_thread_ptr, &thread_variables_to_index_map).unwrap();

        let (packet_id, packet) = reader1.get_latest_packet(&None).unwrap();
        assert_eq!(packet_id, PacketId(33));
        assert_eq!(packet, ReadVariableSetXY{ timestamp: 1.25, robot_mode: 7 });

        let (packet_id, packet) = reader2.get_latest_packet(&None).unwrap();
        assert_eq!(packet_id, PacketId(33));
        assert_eq!(packet, ReadVariableSetYZ{ robot_mode: 7, payload: 7.5 });
    }
}