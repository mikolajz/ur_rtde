    ...
    T: 'static + RawRTDETrait + Send,
{
    ...
    pub fn start(
        ...
        let shared_state_lock = Arc::new(Mutex::new(SharedState {
            stop_request: false,
            latest_packet: None,
        }));
            ...
            loop {
...

                {
                    ...
                    if let Some(robot_outputs) = opt_robot_outputs {
                        shared_state.latest_packet = Some(Packet {
                ...
                for write_request in write_receiver.try_iter() {
                    write_request
                        .result_sender
                        .send(raw_rtde.send_robot_input(&input_io_setup, &write_request.data))
    ...
    T: 'static + RawRTDETrait + Send,
{
    fn stop(&mut self) -> Result<(), String> {
        ...
        if let Some(join_handle) = taken_join_handle {
            ...
            let _ = join_handle
                .join()
                .map_err(|e| format!("Got error from receiver thread: {:?}", e))?;
    ...
    fn get_latest_packet(&self, previous_id: &Option<PacketId>) -> Result<Packet, String> {
        ...
        let (shared_state, timeout) = self
            .condition
            .wait_timeout_while(shared_state, RECEIVE_TIMEOUT, |shared_state| -> bool {
                !Self::good_latest_packet(previous_id, shared_state)
            })
            .unwrap();
...
mod tests {
    ...
    fn sends_robot_inputs() {
        ...
        assert_eq!(
            ...
            vec![input_packet_0, input_packet_1]
        );
        thread.stop().unwrap();
    }
    ...
    fn receives_robot_outputs() {
        ...
        let read_packet_1 = thread
            .get_latest_packet(&Some(read_packet_0.packet_id))
            .unwrap();
        ...
        let join_handle = thread::spawn(move || {
            ...
            control_copy
                .lock()
                .unwrap()
                .queue_output(0, &output_packet_2_copy);
        });
        let read_packet_2 = thread
            .get_latest_packet(&Some(read_packet_1.packet_id))
            .unwrap();
        ...
        control
            .lock()
            .unwrap()
            .queue_repeating_output(0, &[RTDEData::Double(0.0), RTDEData::Int32(0)]); // Make the thread progress. Right now it's required to make stop() work.
        thread.stop().unwrap();
    }
}
