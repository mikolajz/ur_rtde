...
use crate::raw_rtde::{IoSetup, RTDEData, RawRTDETrait};
...
impl FakeRawRTDEControl {
    ...
    fn queue_output_worker(
        &mut self,
        recipe_id: u8,
        data: &[RTDEData],
        queue_item_mode: QueueItemMode,
    ) {
...
impl FakeRawRTDE {
    ...
    pub fn new_fake_and_control() -> (FakeRawRTDE, Arc<Mutex<FakeRawRTDEControl>>) {
        let control = Arc::new(Mutex::new(FakeRawRTDEControl {
    ...
    fn map_types<U: 'static + AsRef<str>>(
        variables: &[U],
        is_input: bool,
    ) -> Vec<Discriminant<RTDEData>> {
        variables
            .iter()
            .map(|x| Self::map_type(x.as_ref(), is_input))
            .collect()
    }
}
...
impl RawRTDETrait for FakeRawRTDE {
        ...
        assert!(
            io_setup
                == &control
                    .input_io_setup
                    .as_ref()
                    .expect("Attempt to send inputs before configuring them.")
                    .io_setup
        );
    ...
    fn wait_for_robot_output(
        ...
        loop {
            ...
            if let Some((index, item)) = control
                .queued_output_packets
                .iter()
                .enumerate()
                .find(|(_, item)| item.recipe_id == io_setup.recipe_id)
            {
            ...
            let timeout_result;
            (control, timeout_result) = new_output_condvar
                .wait_timeout(control, RECEIVE_DATA_TIMEOUT)
                .unwrap();
...
mod tests {
    ...
    fn stores_inputs_and_outputs() {
        ...
        let inputs_io_setup = fake
            .configure_inputs(&["standard_analog_output_0", "standard_digital_output"])
            .unwrap();
        let outputs_io_setup_1 = fake
            .configure_outputs(500.0, &["timestamp", "robot_mode"])
            .unwrap();
        ...
        assert_eq!(
            ...
            IoSetup {
                recipe_id: 1,
                types: vec![discriminant(&RTDEData::Double(0.0)),]
            }
        );

        assert_eq!(
            control.lock().unwrap().input_io_setup,
            Some(FakeIoConfig {
                vars: vec![
        ...
        assert_eq!(
            control.lock().unwrap().output_io_setups,
            vec![
                FakeIoConfig {
    ...
    fn sends_outputs_queued_early() {
        let (mut fake, control) = FakeRawRTDE::new_fake_and_control();
        let data_packet_0 = [RTDEData::Double(1.0), RTDEData::Int32(3)];
        let data_packet_1 = [RTDEData::Double(2.0), RTDEData::Int32(4)];

        let outputs_io_setup = fake
            .configure_outputs(500.0, &["timestamp", "robot_mode"])
            .unwrap();
        ...
        control
            .lock()
            .unwrap()
            .queue_output(outputs_io_setup.recipe_id, &data_packet_0);
        control
            .lock()
            .unwrap()
            .queue_output(outputs_io_setup.recipe_id, &data_packet_1);
        ...
        let join_handle = {
            ...
            thread::spawn(move || {
                ...
                control_other_ref
                    .lock()
                    .unwrap()
                    .queue_output(outputs_io_setup_copy.recipe_id, &data_packet_0_copy);
                control_other_ref
                    .lock()
                    .unwrap()
                    .queue_output(outputs_io_setup_copy.recipe_id, &data_packet_1_copy);
            })
        };
    ...
    fn accepts_inputs() {
        let (mut fake, control) = FakeRawRTDE::new_fake_and_control();
        let data_packet_0 = [RTDEData::Double(1.0), RTDEData::Uint8(42)];
        let data_packet_1 = [RTDEData::Double(2.0), RTDEData::Uint8(137)];

        let inputs_io_setup = fake
            .configure_inputs(&["standard_analog_output_0", "standard_digital_output"])
            .unwrap();

        fake.send_robot_input(&inputs_io_setup, &data_packet_0)
            .unwrap();
        fake.send_robot_input(&inputs_io_setup, &data_packet_1)
            .unwrap();
        ...
        assert_eq!(
            control.lock().unwrap().sent_input_packets,
            &[data_packet_0, data_packet_1]
        );
    }
}
