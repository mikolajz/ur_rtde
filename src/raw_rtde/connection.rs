...
use crate::raw_rtde::packet_data::{ControllerVersion, IoSetup, RTDEData};
...
pub trait RawRTDETrait {
    ...
    fn configure_inputs<U: 'static + AsRef<str>>(
        &mut self,
        variables: &[U],
    ) -> Result<IoSetup, String>;
    ...
    fn configure_outputs<U: 'static + AsRef<str>>(
        &mut self,
        frequency: f64,
        variables: &[U],
    ) -> Result<IoSetup, String>;
    ...
    fn wait_for_robot_output(
        &mut self,
        io_setup: &IoSetup,
    ) -> Result<Option<Vec<RTDEData>>, String>;

    /// Returns a closure that can be called to interrupt a wait_for_robot_output (happening in another thread).
    fn get_wait_for_robot_output_interrupter(
        &self,
    ) -> Box<dyn Fn() -> Result<(), String> + Send + 'static>;
}
...
impl GenericRawRTDE<TcpStream> {
    ...
    pub fn connect(addr: SocketAddr) -> Result<Self, String> {
        let stream =
            TcpStream::connect(addr).map_err(|e| format!("Error connecting to RTDE: {}", e))?;
...
impl<T: Read + Write + event::Source> GenericRawRTDE<T> {
    ...
    pub fn new(stream: T) -> Result<Self, String> {
        let poll =
            Poll::new().map_err(|e| format!("Error starting RTDE - when create a Poll: {e}"))?;

        let waker = Arc::new(
            Waker::new(poll.registry(), WAKE_TOKEN)
                .map_err(|e| format!("Error creating waker: {e}"))?,
        );
    ...
    pub fn negotiate_protocol_version(&mut self) -> Result<(), String> {
        ...
        match protocol_ok {
            true => Ok(()),
            false => Err(format!(
                "Robot did not accept RTDE protocol version {}",
                RTDE_VERSION
            )),
        }
    }
    ...
    fn decode_protocol_version_reply(payload: Vec<u8>) -> Result<bool, String> {
        ...
        let protocol_ok = rdr
            .read_u8()
            .map_err(|_e| "Protocol version reply too short")?
            != 0;
        Ok(protocol_ok)
    }
    ...
    fn decode_controller_version_reply(payload: Vec<u8>) -> Result<ControllerVersion, String> {
        ...
        let major = rdr
            .read_i32::<BigEndian>()
            .map_err(|_e| "Controller version reply too short")?;
        let minor = rdr
            .read_i32::<BigEndian>()
            .map_err(|_e| "Controller version reply too short")?;
        let bugfix = rdr
    ...
    fn encode_configure_inputs_request<U: AsRef<str>>(variables: &[U]) -> Result<Vec<u8>, String> {
        ...
        if !variables.iter().all(|v| !v.contains(',')) {
            return Err(format!(
                "Comma can't be part of variable name in {:?}",
                variables
            ));
        }
    ...
    fn encode_configure_outputs_request<U: AsRef<str>>(
        frequency: f64,
        variables: &[U],
    ...
    fn decode_configure_io_reply<U: AsRef<str>>(
        payload: Vec<u8>,
        variables: &[U],
    ) -> Result<IoSetup, String> {
        let variables = variables.iter().map(|v| v.as_ref()).collect::<Vec<&str>>();
        ...
        let string = String::from_utf8(payload[1..].to_vec())
            .map_err(|_e| format!("Inputs reply contains non-UTF8 chars: {:?}", &payload[1..]))?;
        ...
        if let Some(offender) = type_names.iter().position(|t| t == "IN_USE") {
            return Err(format!(
                "Variable {} is already in use by another connection",
                variables[offender]
            ));
        }

        let types: Vec<Discriminant<RTDEData>> = type_names
            .iter()
            .map(|n| Self::map_type_name_to_discriminant(n))
            .collect::<Result<Vec<_>, _>>()?;

        Ok(IoSetup { recipe_id, types })
    }

    fn encode_robot_input_packet(
        io_setup: &IoSetup,
        input_data: &[RTDEData],
    ) -> Result<Vec<u8>, String> {
    ...
    fn decode_robot_output_packet(
        payload: &[u8],
        io_setup: &IoSetup,
    ) -> Result<Vec<RTDEData>, String> {
        let mut rdr = Cursor::new(payload);
        let recipe_id = rdr
            .read_u8()
            .map_err(|_e| "Data packet received too short")?;
        if recipe_id != io_setup.recipe_id {
            return Err(format!(
                "Received recipie id {} instead of {}",
                recipe_id, io_setup.recipe_id
            ));
        }
    ...
    fn encode_packet(command: u8, payload: Vec<u8>) -> Result<Vec<u8>, String> {
        let packet_size: u16 = (HEADER_SIZE + payload.len())
            .try_into()
            .map_err(|_e| "Payload too large")?;
    ...
    fn mio_write_all(&mut self, packet: &[u8], deadline: Instant) -> Result<(), String> {
        ...
        self.poll
            .registry()
            .register(&mut self.stream, SOCKET_WAIT_TOKEN, Interest::WRITABLE)
            .map_err(|e| format!("Error sending data - when registering poll event: {e}"))?;
        ...
        while written < packet.len() {
            match self.stream.write(&packet[written..]) {
                ...
                Err(ref e) if e.kind() == ErrorKind::WouldBlock => {
                    self.poll
                        .poll(&mut events, Some(deadline.duration_since(Instant::now())))
                        .map_err(|e| {
                            format!("Error sending data - when polling the socket: {e}")
                        })?;
                }
                Err(e) => {
        ...
        self.poll
            ...
            .map_err(|e| format!("Error sending data - deregistering poll: {e}"))?;
        result
    }

    fn wait_for_reply_interruptible(
        &mut self,
        command: u8,
        allow_waker: bool,
        ...
        loop {
            ...
            let (packet_cmd, payload) = self
                .wait_for_packet(deadline, allow_waker)
                .map_err(|e| format!("Error waiting for reply to command {command}: {e}"))?;
            match packet_cmd {
    ...
    fn wait_for_reply(&mut self, command: u8) -> Result<Vec<u8>, String> {
        self.wait_for_reply_interruptible(command, false)
            .map(|opt_payload| {
                opt_payload.expect("When allow_waker is None, the payload must be set")
            })
    }
    ...
    fn wait_for_packet(
        ...
        match self.mio_read_exact(&mut header, deadline, allow_waker)? {
            ...
            ReadResult::WokenBeforeRead => {
                return Ok((WOKEN_BEFORE_READ_PSEUDO_CMD, Vec::<u8>::new()));
            }
        }
    ...
    fn mio_read_exact(
        ...
        self.poll
            .registry()
            .register(&mut self.stream, SOCKET_WAIT_TOKEN, Interest::READABLE)
            .map_err(|e| format!("Error receiving data - when registering poll event: {e}"))?;
        ...
        while read < buf.len() {
            match self.stream.read(&mut buf[read..]) {
                ...
                Err(ref e) if e.kind() == ErrorKind::WouldBlock => {
                    ...
                    self.poll.poll(&mut events, Some(timeout)).map_err(|e| {
                        format!("Error sending data - when polling the socket: {e}")
                    })?;
        ...
        self.poll
            .registry()
            .deregister(&mut self.stream)
            .map_err(|e| format!("Error receiving data - deregistering poll: {e}"))?;
        result
    }
    ...
    fn decode_header(header: [u8; HEADER_SIZE]) -> Result<(u8, u16), String> {
        ...
        let payload_size: u16 = packet_size
            .checked_sub(HEADER_SIZE as u16)
            .ok_or_else(|| format!("Illegal packet size {}", packet_size))?;
...
impl<T: Read + Write + event::Source> RawRTDETrait for GenericRawRTDE<T> {
    ...
    fn configure_outputs<U: AsRef<str>>(
        &mut self,
        frequency: f64,
        variables: &[U],
    ) -> Result<IoSetup, String> {
    ...
    fn wait_for_robot_output(
        ...
        let opt_reply =
            self.wait_for_reply_interruptible(RTDECommand::RtdeDataPacket as u8, true)?;
        match opt_reply {
    ...
    fn get_wait_for_robot_output_interrupter(
        ...
        Box::new(move || {
            waker
                .wake()
                .map_err(|e| format!("Error waking the RTDE thread: {e}"))
        })
    }
...
mod tests {
    ...
    fn test_configure_inputs() {
        ...
        tester_end
            .write_all(&[
                0,
                (4 + types_string.len()).try_into().unwrap(), // u16 BigEndian packet size
                b'I' as u8,                                   // Command
                23,                                           // Recipie ID
            ])
            .unwrap();
    ...
    fn test_start_robot_to_host_transmition() {
        ...
        rtde.start_robot_to_host_transmition()
            .expect("Function failed");
        let mut written = Vec::<u8>::new();
        assert_eq!(
            tester_end.read_to_end(&mut written).map_err(|e| e.kind()),
    ...
    fn test_pause_robot_to_host_transmition() {
        ...
        rtde.pause_robot_to_host_transmition()
            .expect("Function failed");
        let mut written = Vec::<u8>::new();
        assert_eq!(
            tester_end.read_to_end(&mut written).map_err(|e| e.kind()),
    ...
    fn test_send_robot_inputs() {
        let (mut tester_end, sut_end) = UnixStream::pair().unwrap();
        let input_data = [RTDEData::Uint32(1), RTDEData::Vector3Int32([2, 3, 4])];
        ...
        rtde.send_robot_input(&io_setup, &input_data)
            .expect("Function failed");
        let mut written = Vec::<u8>::new();
        assert_eq!(
            tester_end.read_to_end(&mut written).map_err(|e| e.kind()),
    ...
    fn test_parse_outpute_packet() {
        ...
        let result = rtde
            .wait_for_robot_output(&io_setup)
            .expect("Function failed");

        assert_eq!(result, Some(vec![RTDEData::Uint8(7), RTDEData::Int32(23),]))
    }
