...
impl Display for ControllerVersion {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        write!(f, "{}.{}.{}.{}", self.major, self.minor, self.bugfix, self.build)
    }
}
...
impl RTDEData {
    ...
    pub fn parse(input: &mut Cursor<&[u8]>, data_type: Discriminant<RTDEData>) -> Result<RTDEData, String> {
        // Can this be replaced with `match`?
        if data_type == discriminant(&RTDEData::Int32(0)) {
            Ok(RTDEData::Int32(input.read_i32::<BigEndian>().map_err(|_e| "Data packet too short")?))
            ...
            Ok(RTDEData::Double(input.read_f64::<BigEndian>().map_err(|_e| "Data packet too short")?))
        } else if data_type == discriminant(&RTDEData::Uint64(0)) {
            Ok(RTDEData::Uint64(input.read_u64::<BigEndian>().map_err(|_e| "Data packet too short")?))
        } else if data_type == discriminant(&RTDEData::Uint8(0)) {
            Ok(RTDEData::Uint8(input.read_u8().map_err(|_e| "Data packet too short")?))
        } else {
